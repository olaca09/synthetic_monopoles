...
def lineplot(sol, initpos, initvel, swarmnum, n, norot, nosyn, difference, alternatestreams, quiver,
    ...
    plt.savefig(f'''saves/graphs/field{field}nr{nr}lablength{lablength}tmax{tmax}J{J}Gamma{Gamma}mass{mass0}len{len0}n{n}pos({initpos[0]:.2f}, {initpos[1]:.2f}, {initpos[2]:.2f}, {initpos[3]:.2f}, {initpos[4]:.2f})vel{initvel}swarmnum{swarmnum}norot{norot}nosyn{nosyn}diff{difference}altstream{alternatestreams}quiver{quiver}parameter{parameter}.png''', bbox_inches='tight')
