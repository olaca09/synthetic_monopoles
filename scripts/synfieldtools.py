...
steptheta = np.pi/ntheta #Step size of theta
...
def diffhamiltonian(diffpar, point, field):
    ...
    if(diffpar == "r"):
        ...
        xi = -1 * Gamma * Bgrid[1,1,1]/(J) #Value of xi at point
        ...
        omega = (np.exp(-1j*phi)/np.sqrt(2) * (dxidr*sin-
                1j*dphdr*xi*sin+dthdr*xi*cos)) #Calculates offdiagonals
        diff_hamx[0,0], diff_hamy[0,0], diff_hamz[0,0] = (-dxidr*cos + xi*dthdr*sin)
        ...
        diff_hamx[2,2], diff_hamy[2,2], diff_hamz[2,2] = (dxidr*cos - xi*dthdr*sin)

        #Return the matrices with correct prefactors
        diff_hamx = J*hbar*diff_hamx
...
def eigensolver(point, field):
    ...
    sinr = np.sin(2*thetar) #Calculate the sine of twice theta_r
    sinr2 = np.sin(thetar)**2 #Calculate the square of sine of theta_r
    ...
    ham[0,0] = cosr**2 - xi*cosB
    ham[0,1] = xi/(sq*expB)*sinB - sinr/(sq*expr)
    ham[0,2] = sinr2/expr**2
    ham[1,0] = xi*expB*sinB/sq - expr*sinr/sq
    ham[1,1] = -np.cos(2*thetar)
    ham[1,2] = xi*sinB/(sq*expB) + sinr/(sq*expr)
    ham[2,0] = expr**2*sinr2
    ham[2,1] = xi*expB*sinB/sq + expr*sinr/sq
    ham[2,2] = xi*cosB-cosr**2
...
def scalarcalc(point, field, n):
    ...
    for i in range(5):
        for l in range(3):
            if l == n: #Remove diagonals
                break
            Phi += (hbar**2 /(2*mass[i]) * #Add up contributions to the synthetic scalar
            (np.vdot(eigvec[n], np.dot(dHam[i], eigvec[l]))*np.vdot(eigvec[l],
                np.dot(dHam[i], eigvec[n])) / (energies[n] - energies[l])**2)).real 
            #Note the cast to real numbers, it's probably numerics raising errors otherwise
    #print(f'Phi = {Phi}')
    ...
    for i in range(5):
        for j in range(5):
            for l in range(3):
                ...
                Fa[i] += (-2*hbar * vel[i] / (energies[n]-energies[l])**2 *
