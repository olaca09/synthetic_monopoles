...
def lineplot(sol, initvel, swarmnum, n, norot, nosyn, alternatestreams):
    ...
    xx, yy, zz = np.mgrid[0:11, 0:11, 0:11]
    xx = lablength/10 * xx
    yy = lablength/10 * yy
    zz = lablength/10 * zz
...
#    Bz = field[2,:,:,:][0::int(nr/5), 0::int(nr/5), 0::int(nr/5)]
    ...
    plt.savefig(f'''saves/graphs/field{field}nr{nr}lablength{lablength}tmax{tmax}J{J}Gamma{Gamma}mass{mass0}len{len0}n{n}vel{initvel}swarmnum{swarmnum}norot{norot}nosyn{nosyn}altstream{alternatestreams}.png''', bbox_inches='tight')
